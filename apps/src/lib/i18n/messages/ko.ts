...
export const KO_MESSAGES: MessageCatalog = {
  ...
  "模型变更会更新网关模式使用的独立目录，不会自动覆盖 `~/.codex/models_cache.json`；运行中的 Codex 仍需重载后生效。直连模式的模型列表与缓存由 Codex 官方管理；Web 端仍可用上方导出按钮下载兼容缓存。":
    "모델 변경 사항은 게이트웨이 모드의 별도 카탈로그에 반영되며 `~/.codex/models_cache.json`을 자동으로 덮어쓰지 않습니다. 실행 중인 Codex에는 다시 로드한 뒤 적용됩니다. 직결 모드의 모델 목록과 캐시는 공식 Codex가 관리하며, Web에서는 위의 내보내기 버튼으로 호환 캐시를 다운로드할 수 있습니다.",
  当前环境不支持浏览器导出:
