...
import { EN_KIRO_MESSAGES } from "./sections/en-kiro";
import { EN_GROK_MESSAGES } from "./sections/en-grok";
import { EN_AGENT_IDENTITY_MESSAGES } from "./sections/en-agent-identity";
import { EN_PROXIES_MESSAGES } from "./sections/en-proxies";
...
export const EN_MESSAGES: MessageCatalog = {
  ...
  智能路由: "Smart routing",
  智能路由口: "Smart routing",
  "统一管理智能别名、批次轮询、额度保护和模型映射入口。": "Unified entry for smart aliases, batch rotation, quota protection, and model mapping.",
  "连接 CodexManager 服务后才能调整智能路由。": "Connect to CodexManager before adjusting smart routing.",
  顺序优先: "Ordered",
  均衡轮询: "Balanced rotation",
  ...
  "前台可请求 smart、coding、fast、cheap，后台按健康、额度、能力和批次策略选路。": "Clients can request smart, coding, fast, or cheap; the backend routes by health, quota, capability, and batch policy.",
  "新线程优先分配到当前承载更少线程的账号，已有线程保持粘性。": "New threads prefer accounts carrying fewer threads while existing threads stay sticky.",
  别名能力口: "Alias capability entry",
  分批正向循环: "Forward batch cycle",
  "按你设定的批次运行：当前批次不可用后才切下一批，不额外打乱。": "Runs by your configured batches: switches only after the current batch is unavailable, without extra shuffling.",
  启用批次轮询: "Enable batch rotation",
  "适合 5 小时限制账号池正向循环。": "Suitable for forward cycling account pools with five-hour limits.",
  "默认关闭；开启后低于保留百分比的账号会被跳过。": "Off by default; when enabled, accounts below the reserved percentage are skipped.",
  启用额度保护: "Enable quota protection",
  刷新配置: "Refresh config",  导入凭据: "Import credentials",
  统一导入凭据: "Unified credential import",
  "一个入口自动识别 Codex Token JSON、Kiro JSON 和 Grok 文本卡密。": "One entry automatically detects Codex token JSON, Kiro JSON, and Grok text cards.",
  "可以选择多个 JSON/TXT 文件，也可以直接粘贴内容；系统会自动分流到对应资源池。": "Select multiple JSON/TXT files or paste content directly; the system routes each item to the right resource pool.",
  导入入口: "Import entry",
  选择文件: "Choose files",
  自动识别: "Auto-detect",
  确认导入全部: "Import all",
  识别结果: "Detection results",
  "只展示平台、数量、置信度和脱敏摘要；不会回显 Token、密码或 SSO。": "Only platform, count, confidence, and masked summaries are shown; tokens, passwords, and SSO are never echoed.",
  识别平台: "Detected platform",
  数量: "Count",
  摘要: "Summary",
  未识别: "Unrecognized",
  不可导入: "Not importable",
  可导入: "Importable",
  等待选择文件或粘贴内容: "Waiting for files or pasted content",
  未找到可导入文件: "No importable files found",
  没有识别到可导入凭据: "No importable credentials detected",
  已识别: "Detected",
  空内容: "Empty content",
  未能识别平台: "Unable to detect platform",
  "粘贴 Codex / Kiro JSON，或 Grok account----password----SSO 文本": "Paste Codex / Kiro JSON, or Grok account----password----SSO text",  导入账号: "import accounts",
  ...
  重置权益: "Reset entitlement",
  重置卡: "Reset card",
  消息重置权益: "Message reset entitlement",
  额度重置权益: "Quota reset entitlement",
  "这里只展示识别到的重置卡；实际调用仍按你设置的批次循环运行。": "This only displays detected reset cards; actual routing still follows your configured batch cycle.",
  需要手动激活: "Manual activation required",
  可用权益: "Available entitlement",
  张: "card(s)",
  过期时间: "Expires at",
  生效时间: "Starts at",
  适用范围: "Applies to",
  "可用{count}张重置卡": "{count} reset card(s) available",
  ...
  模型展示方式: "Model visibility",
  前台可选择跨平台模型: "Frontend can choose cross-platform models",
  后台托管选择模型: "Backend-managed model selection",
  "前台只看到统一别名或套餐模型；真实 Codex/Kiro/Grok 模型由后台调度。": "The frontend only sees unified aliases or package models; real Codex/Kiro/Grok models are scheduled by the backend.",
  "前台可以看到允许范围内的跨平台模型，并自行选择。": "The frontend can see allowed cross-platform models and choose them directly.",
  "GPT / Codex 池": "GPT / Codex Pool",
  "Kiro 池": "Kiro Pool",
  "Grok 池": "Grok Pool",
  总数: "Total",
  限额: "Limited",
  开: "On",
  关: "Off",
  参与调用: "Calling enabled",
  暂停调用: "Calling paused",
  平台池总开关: "Pool switch",
  总开关待接入: "Switch pending",
  池子详情: "Pool details",
  进入高级管理: "Advanced management",
  暂无资源: "No resources",
  未探测: "Not probed",
  "OpenAI/Codex 账号池，按批次循环参与统一网关调用。": "OpenAI/Codex account pool participates in unified gateway calls by batch cycle.",
  "Kiro 凭据池，可独立启停并参与 OpenAI 格式统一路由。": "Kiro credential pool can be toggled independently and join OpenAI-format unified routing.",
  "Grok Web 账号池，后续可加入同一套批次和平台开关。": "Grok Web account pool can later join the same batch and platform switch system.",
  资源池总控: "Resource pool control",
  统一资源池: "Unified resource pools",
  "先看平台池，再展开账号方块；表格管理保留在高级页面。": "Start from platform pools, then expand account tiles; table management stays in advanced pages.",
  批次轮询优先: "Batch rotation first",
  ...
  "一个 OpenAI Key，统一调度 Codex 与 Kiro 可用模型":
    "One OpenAI key orchestrates every available Codex and Kiro model",
  独立账号池与模型目录: "Independent account pool and model catalog",
  健康账号参与实时路由: "Healthy accounts participate in live routing",
  "协议转换、能力匹配与故障切换": "Protocol conversion, capability matching, and failover",
  "OpenAI Chat 与 Responses 双入口": "OpenAI Chat and Responses endpoints",
  凭据级模型探测与加密存储: "Credential-level model detection and encrypted storage",
  个已验证可用模型: "verified available models",
  已验证模型: "Verified models",
  正在读取模型状态: "Reading model status",
  "导入 Kiro 凭据后自动探测": "Models are detected after importing Kiro credentials",
  凭据中心: "Credential center",
  "平台 Key": "Platform key",
  上游资源层: "Upstream resources",
  路由核心层: "Routing core",
  统一访问层: "Unified access",
  账号直连: "Direct account",
  "健康账号 / 总账号": "Healthy / total accounts",
  "Kiro 已验证模型": "Verified Kiro models",
  资源健康: "Resource health",
  用量与趋势: "Usage and trends",
  "账号稳定 · 分批正向循环": "Account stability · forward batch rotation",
  "把账号池固定分组；当前组全部限额或冷却后才切换下一组，末组结束后从首组检查恢复。":
    "Keep accounts in fixed groups; advance only when the current group is exhausted or cooling down, then check the first group again after the last group.",
  每批账号数量: "Accounts per batch",
  "例如填 5：账号 1–5、6–10、11–15 依次成组。":
    "For example, 5 groups accounts as 1–5, 6–10, and 11–15.",
  "无重置时间时的恢复窗口（分钟）": "Fallback recovery window (minutes)",
  单次请求最多尝试: "Maximum attempts per request",
  "限制换连接、重试和切号的总次数，避免异常时扫完整个号池。":
    "Caps reconnects, retries, and account switches so an incident cannot sweep the entire pool.",
  当前批次: "Current batch",
  本批可用: "Available in batch",
  最早恢复: "Earliest recovery",
  等待真实额度重置时间: "Waiting for a quota reset time",
  "默认关闭，不影响现有选路。分批只限定候选范围，批内仍使用健康度、额度、并发和会话粘性策略。":
    "Disabled by default. Batching only limits the candidate range; health, quota, concurrency, and session affinity still apply within a batch.",
  ...
  "中转站 KEY": "Upstream API key",
  "一个入口自动识别 Codex Token JSON、Kiro JSON、Grok 文本卡密和中转站 KEY。": "One import entry automatically recognizes Codex Token JSON, Kiro JSON, Grok credential text, and upstream API keys.",
  "粘贴 Codex / Kiro JSON、Grok account----password----SSO，或中转站 URL + sk-KEY": "Paste Codex / Kiro JSON, Grok account----password----SSO, or an upstream URL + sk-KEY",
  "自动重探中": "Automatic retry scheduled",
  "网络出口被 Cloudflare 拦截": "Network exit blocked by Cloudflare",
  "系统将在30秒、2分钟、5分钟后自动重探；切换出口后也可立即探测。": "The system will retry automatically after 30 seconds, 2 minutes, and 5 minutes. You can also probe immediately after changing the network exit.",
};
