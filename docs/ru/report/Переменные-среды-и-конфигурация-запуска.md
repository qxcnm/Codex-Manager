# Переменные среды и конфигурация запуска
...
- `CODEXMANAGER_LOG`: фильтр журналирования; значение по умолчанию — `info`. Service, web и start поддерживают выражения `env_logger`, например `info,codexmanager_service=debug`; настольное приложение принимает один уровень: `trace`, `debug`, `info`, `warn`, `error` или `off`.
- `CODEXMANAGER_LOG_STYLE`: стиль журнала для service, web и start из набора значений `env_logger`.
- `RUST_LOG` / `RUST_LOG_STYLE`: стандартные совместимые переменные для Rust-процессов, кроме настольного приложения. Если они заданы, то имеют приоритет над соответствующими переменными `CODEXMANAGER_*`.

Настольное приложение записывает Rust-журналы в подкаталог `logs` каталога данных приложения и передаёт туда глобальные ошибки фронтенда, необработанные отклонения Promise и ошибки ключевых mutation. Сообщения об исключениях ограничиваются по длине, а распространённые поля токенов маскируются. В Windows путь по умолчанию — `%APPDATA%\com.codexmanager.desktop\logs\CodexManager.log` (Roaming); `%LOCALAPPDATA%` больше не используется.
...
- `CODEXMANAGER_UPSTREAM_PROXY_URL`
- `CODEXMANAGER_UPSTREAM_PROXY_BYPASS_HOSTS`
- `CODEXMANAGER_UPSTREAM_TOTAL_TIMEOUT_MS`: общий таймаут gateway-запроса в миллисекундах. Значение по умолчанию `0` означает, что service не обрывает запрос по общей длительности.
- `CODEXMANAGER_UPSTREAM_STREAM_TIMEOUT_MS`
- `CODEXMANAGER_SSE_KEEPALIVE_ENABLED`: включает комментарии keepalive в исходящем SSE-потоке. Значение по умолчанию `1`; `0` отключает keepalive, не меняя настроенный интервал. Параметр действует только для потоковых SSE-ответов; непотоковые Images JSON-ответы требуют достаточно больших read timeout на клиенте и reverse proxy.
- `CODEXMANAGER_SSE_KEEPALIVE_INTERVAL_MS`
...
- `CODEXMANAGER_CODEX_IMAGE_GENERATION_ENABLED`: включает совместимый путь генерации изображений. Значение по умолчанию `1`.
- `CODEXMANAGER_CODEX_IMAGE_GENERATION_AUTO_INJECT_TOOL`: управляет автоматическим добавлением tool `image_generation` в обычные `/v1/responses` запросы. Значение по умолчанию `0` отключает добавление; при `1` tool добавляется только если клиент не передал его явно. Явно переданный клиентом tool не дублируется.
- `CODEXMANAGER_CODEX_IMAGE_MAIN_MODEL`: основная conversation-модель, используемая внутри Images API compatibility endpoints. Значение по умолчанию `gpt-5.4-mini`.
- `CODEXMANAGER_CODEX_IMAGE_TOOL_MODEL`: модель image tool. Значение по умолчанию `gpt-image-2`.

Примечания:

- `/v1/images/generations` и `/v1/images/edits` внутри преобразуются в цепочку `/v1/responses + image_generation tool`.
- Если Codex CLI нативно отправляет `tools[].type = "image_generation"`, gateway передаёт его дальше и не меняет существующий текстовый путь запросов.
...
1. Проверьте, не слишком ли мал `CODEXMANAGER_UPSTREAM_STREAM_TIMEOUT_MS`, из-за чего длинный streaming-запрос обрывается раньше времени.
2. Проверьте, не задан ли `CODEXMANAGER_UPSTREAM_TOTAL_TIMEOUT_MS` коротким ненулевым значением, из-за чего service обрывает запрос по общей длительности.
3. Проверьте, не отключён ли `CODEXMANAGER_SSE_KEEPALIVE_ENABLED` и не слишком ли велик `CODEXMANAGER_SSE_KEEPALIVE_INTERVAL_MS`, из-за чего промежуточный proxy считает SSE idle.
4. Проверьте timeout клиента и idle-лимиты proxy. В логах запросов `120s/1.8s` означает общую длительность / время до первого ответа и не обязательно означает, что service total timeout равен `120s`.
...
- 架构说明：[Архитектура.md](../Архитектура.md)
- 发布说明：[Выпуск-и-артефакты.md](../release/Выпуск-и-артефакты.md)
