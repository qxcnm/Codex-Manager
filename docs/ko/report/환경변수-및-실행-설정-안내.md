...
- `CODEXMANAGER_LOG`: 기본값은 `info`입니다. service, web, start는 `info,codexmanager_service=debug` 같은 `env_logger` 필터 표현식을 지원하며, 데스크톱 앱은 `trace`, `debug`, `info`, `warn`, `error`, `off` 중 하나의 레벨을 지원합니다.
- `CODEXMANAGER_LOG_STYLE`: service, web, start에서 사용하는 `env_logger` 로그 스타일입니다.
- `RUST_LOG` / `RUST_LOG_STYLE`: 비데스크톱 Rust 프로세스의 표준 호환 변수이며, 설정된 경우 대응하는 `CODEXMANAGER_*` 변수보다 우선합니다.

데스크톱 앱은 Rust 로그를 애플리케이션 데이터 디렉터리의 `logs` 하위 디렉터리에 저장하고 전역 프런트엔드 오류, 처리되지 않은 Promise 거부, 주요 mutation 실패를 전달합니다. 예외 메시지는 길이가 제한되고 일반적인 토큰 필드는 마스킹됩니다. Windows 기본 경로는 `%APPDATA%\com.codexmanager.desktop\logs\CodexManager.log`(Roaming)이며 더 이상 `%LOCALAPPDATA%`를 사용하지 않습니다.
...
- `CODEXMANAGER_UPSTREAM_STREAM_TIMEOUT_MS`
- `CODEXMANAGER_SSE_KEEPALIVE_ENABLED`: 다운스트림 SSE 주석 keepalive 전송 여부입니다. 기본값은 `1`이며, `0`으로 설정하면 간격 설정은 유지한 채 keepalive만 비활성화합니다. 스트리밍 SSE 응답에만 적용되므로 비스트리밍 Images JSON 응답에는 충분히 긴 클라이언트 및 reverse proxy read timeout이 필요합니다.
- `CODEXMANAGER_SSE_KEEPALIVE_INTERVAL_MS`
...
- `CODEXMANAGER_CODEX_IMAGE_GENERATION_AUTO_INJECT_TOOL`: 일반 `/v1/responses` 요청에 `image_generation` tool을 자동 주입할지 여부. 기본값 `0`은 주입하지 않으며, `1`로 설정하면 클라이언트가 명시적으로 tool을 보내지 않은 경우에만 자동 주입합니다. 클라이언트가 명시적으로 보낸 tool은 중복 주입하지 않습니다.
...
3. `CODEXMANAGER_SSE_KEEPALIVE_ENABLED`가 꺼져 있거나 `CODEXMANAGER_SSE_KEEPALIVE_INTERVAL_MS`가 너무 길어 중간 프록시가 SSE를 idle로 판단하는지 확인합니다.
