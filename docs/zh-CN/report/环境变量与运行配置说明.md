...
- `CODEXMANAGER_LOG`：日志过滤器，默认 `info`。service、web 和 start 支持 `env_logger` 过滤表达式（例如 `info,codexmanager_service=debug`）；桌面端支持单一日志级别 `trace`、`debug`、`info`、`warn`、`error` 或 `off`。
- `CODEXMANAGER_LOG_STYLE`：service、web 和 start 的日志样式，可使用 `env_logger` 支持的样式值。
- `RUST_LOG` / `RUST_LOG_STYLE`：非桌面 Rust 进程的标准兼容变量；存在时优先于对应的 `CODEXMANAGER_*` 变量。

桌面端会将 Rust 日志写入应用数据目录下的 `logs` 子目录，并接收前端全局错误、未处理 Promise 拒绝和关键 mutation 失败；异常消息会做长度限制和常见令牌字段脱敏。Windows 下的默认路径为 `%APPDATA%\com.codexmanager.desktop\logs\CodexManager.log`（Roaming），不再使用 `%LOCALAPPDATA%`。
...
- `CODEXMANAGER_UPSTREAM_STREAM_TIMEOUT_MS`
- `CODEXMANAGER_USE_WEBSOCKET_UPSTREAM`：是否让 ChatGPT `/v1/responses` 流式上游优先尝试 WebSocket 传输；默认 `0`。这是实验开关，失败会回退 HTTP 流式路径，会沿用上游代理与连接超时配置。
...
- `CODEXMANAGER_CODEX_IMAGE_GENERATION_AUTO_INJECT_TOOL`：普通 `/v1/responses` 请求是否自动注入 `image_generation` tool，默认 `0` 不注入；填 `1` 时会在客户端未显式传入 tool 时自动注入，显式由客户端传入 tool 时不重复注入。
