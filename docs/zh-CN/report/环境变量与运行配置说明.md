...
- `CODEXMANAGER_GROK_STATSIG_SIGNER_URL`：Grok Web 的 `x-statsig-id` 签名服务地址。默认使用固定协议参考项目的 HTTPS signer；也可设置为本机 `http://127.0.0.1:<port>/sign` 或 `http://localhost:<port>/sign`。
- 远程 signer 只接收请求方法、路径以及 Grok 首页公开的 verification meta，不接收账号、密码、SSO Cookie 或平台 Key。
- Grok 凭据自身的代理出口在凭据中心单独保存；模型探测会沿用该出口及固定浏览器 Header，避免账号在不同出口之间随机漂移。
- 模型探测只在用户主动点击时运行。无法确认套餐等级时不会猜测，也不会把未验证模型加入可用列表。
