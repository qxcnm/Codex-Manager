...
async fn refresh_idc_token(
    ...
    let os_name = credentials
        .system_version
        .as_deref()
        .unwrap_or(&config.system_version);
    let node_version = credentials
        .node_version
        .as_deref()
...
pub struct MultiTokenManager {
    ...
    /// Optional host integration hook. CodexManager uses this to persist
    /// refreshed tokens back to its encrypted database instead of kiro.rs files.
    credential_update_handler: Option<std::sync::Arc<dyn Fn(u64, &KiroCredentials) + Send + Sync>>,
}
...
impl MultiTokenManager {
    ...
    pub fn new(
        ...
        let manager = Self {
            ...
            credential_update_handler: None,
        };
    ...
    pub fn with_credential_update_handler(
        mut self,
        handler: std::sync::Arc<dyn Fn(u64, &KiroCredentials) + Send + Sync>,
    ) -> Self {
        self.credential_update_handler = Some(handler);
        self
    }

    fn notify_credential_updated(&self, id: u64, credentials: &KiroCredentials) {
        if let Some(handler) = &self.credential_update_handler {
            handler(id, credentials);
        }
    }
    ...
    async fn try_ensure_token(
        ...
        let creds = if needs_refresh {
            ...
            if is_token_expired(&current_creds) || is_token_expiring_soon(&current_creds) {
                ...
                self.notify_credential_updated(id, &new_creds);
    ...
    pub async fn force_refresh_token_for(&self, id: u64) -> anyhow::Result<()> {
        ...
        if let Some(updated) = updated.as_ref() {
            self.notify_credential_updated(id, updated);
        }
