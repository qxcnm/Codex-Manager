...
pub fn map_model(model: &str) -> Option<String> {
    ...
    if model_lower.contains("sonnet") {
        if model_lower.contains("4-6") || model_lower.contains("4.6") {
            ...
            // Kiro's broadly available Sonnet fallback is 4.5. Older Anthropic
            // aliases (including Claude 3.5 and the unqualified Sonnet 4 name)
            // are normalized to it instead of selecting the newer 4.6 model,
            // which may not be enabled for every credential tier.
        ...
        if model_lower.contains("4-5") || model_lower.contains("4.5") {
            ...
            // Preserve compatibility with Anthropic's dated/unqualified Opus
            // 4 aliases while using Kiro's conservative supported baseline.
            Some("claude-opus-4.5".to_string())
        }
